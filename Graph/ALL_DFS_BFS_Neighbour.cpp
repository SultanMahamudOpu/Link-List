...
public:
    ...
    void DFS(int source)
    {
        static vector<bool> visited(V, false); // static vector for not change this vector before end DFS function
        visited[source] = true;                // first need to make it true

        cout << source << " "; // print value

        for (auto element : adj[source])
        {
            int v = element.first; // capture first element
            if (visited[v] != true)
            {
                DFS(v); // recuraceve call
            }
        }
    }
};
...
{
    cout << "..............................." << endl;
    cout << "Enter you Graph's Value:" << endl;
    int V, E, source;
    cin >> V >> E >> source;
    Graph g(V);
    for (int i = 0; i < E; i++)
    ...
    while (choice != 0)
    {
        cout << endl
             << endl;
        cout << "1.Print Neabor" << endl
             << "2.Print Breath First Search" << endl
             << "3.Print Deapth Fist Search" << endl
             << "4.Exit" << endl;
